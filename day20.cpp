/*
	---------------REGULAR EXPRESSIONS----------------
	defined in header #include <regex>

	header defines regex -- class that represents a regular expression
	...
	this pattern says we want to find any string that does not begin with c
	and is followed by ei in that order! This string also describes strings
	containing exactly three characters. We want the entire word that contains 
	this pttern. To match teh word, we need a regular expression that will match the letters
	that come before and after our three-letter pattern. 
	If we want the whole word, the pattern must be updated...
